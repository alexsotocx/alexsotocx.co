...
```
The important part of the last code is ```<Link to={`/blog/${post.folder}`} className="full-container-link"></Link>```. There is the connection  between pages, that line generate a link to the post.
